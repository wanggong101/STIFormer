...
def loadGraph(spatial_graph_path):
    try:
        adj = np.load(spatial_graph_path)
        # 确保路径存在
        # if os.path.exists(spatial_graph_path):
        #     with open(spatial_graph_path, "rb") as f:
        #         adj= pickle.load(f)
    except Exception as e:
        raise ValueError(f"无法加载 .npy 文件: {spatial_graph_path}. 错误信息: {e}")
    adj = adj + np.eye(adj.shape[0])
    sampled_nodes_number = int(math.log2(adj.shape[0]))
