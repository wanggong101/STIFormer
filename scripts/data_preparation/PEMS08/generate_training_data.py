...
def generate_data(args: argparse.Namespace):
    ...
    last_20_percent_samples = int(total_samples * 0.2)
    # 提取最后 20% 的数据
    data_last_20_percent = processed_data[-last_20_percent_samples:]
    # 每个样本包含 12 个时间步
    time_steps_per_sample = 12
    # 计算可以拆分出多少个样本
    num_samples = data_last_20_percent.shape[0] // time_steps_per_sample
    # 通过 reshape 拆分数据
    # 这里我们需要调整维度：每 12 个时间步为一个样本
    data_reshaped = data_last_20_percent[:num_samples * time_steps_per_sample]  # 截取能整除的部分
    data_reshaped = data_reshaped.reshape(num_samples, time_steps_per_sample, data_last_20_percent.shape[1],
                                          data_last_20_percent.shape[2])

    # X 是当前时间步的 12 个时间步，Y 是下一个 12 个时间步
    x = data_reshaped[:-1]  # X 是前 12 个时间步
    y = data_reshaped[1:]  # Y 是后 12 个时间步

    # 保存 X 和 Y 到 data.npz 文件，使用合适的键
    np.savez('data08.npz', processed_x=x, processed_y=y)
    # save data
    index = {}
